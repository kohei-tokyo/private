...
def tensor_ssim(img1, img2):
    ...
    return 2.0 - ssim(img1, img2, data_range=1.0, size_average=False)
...
class DDPM(nn.Module):
    def __init__(self, config: DDPMConfig, *args, **kwargs):
        ...
        self.condition_concat = self.config.condition_concat
        ...
        if self.model_unet == "original":
            ...
            self.model = UNet_gemini(
                ...
                num_classes=self.target_class,
                condition_concat=self.condition_concat,
                device=self.device
            ).to(self.device)
        else:
    ...
    def make_loader(self, folders, mode):
        ...
        if mode == "train":
            ...
            return DataLoader(combined_dataset, batch_size=self.batch_size, shuffle=False,
                              num_workers=self.num_workers, pin_memory=True, persistent_workers=self.num_workers > 0)
    ...
    def calc_batch(self, ph1, ph2, real, c, mode):
        if mode == "train":
            return self.calc_matrix_train(ph1, ph2, real, c)
        else:
            metrics_dict = None
            len_met = 0
            _, _, H, W = ph1.shape
    ...
    def calc_matrix_train(self, ph1, ph2, real, c):
        ...
        if np.random.random() < self.no_label:
            ...
            label_train = False
        if self.real_pred:
            prediction = self.model(x_t, t, x, c)
    ...
    def calc_matrix_test(self, ph1, ph2, real, c):
        x = torch.concat([ph1, ph2], dim=1).to(self.device)
        real = real.to(self.device)
        c = c.to(self.device)

        sampled_images = self.diffusion.sample(
            self.model, labels=x, c=c,
            num_inference_steps=self.num_inference_steps, noise_add=self.noise_add, cfg_scale=self.cfg_scale
        )
        ...
        lpips_loss = lpips_calc_each(real, sampled_images, c, self.target, self.loss_fn_lpips, "lpips")

        ema_sampled_images = self.diffusion.sample(
            self.ema_model, labels=x, c=c,
            num_inference_steps=self.num_inference_steps, noise_add=self.noise_add, cfg_scale=self.cfg_scale
        )
        ...
        lpips_loss_ema = lpips_calc_each(real, ema_sampled_images, c, self.target, self.loss_fn_lpips, "lpips_ema")
    ...
    def show_result(self, mode):
        ...
        for ph1, ph2, real, c in loader:
            if condition_n_x[c[0].item()] < n:
                output_pred, output_pred_ema, real = self.make_prediction(mode, ph1, ph2, real, c)
                for i in range(len(output_pred)):
                    if condition_n_x[c[i].item()] < n:
                        self.show_images(real[i], output_pred[i], output_pred_ema[i], c[i].item(), mode, condition_n_x)
                        condition_n_x[c[i].item()] += 1

    def make_prediction(self, mode, ph1, ph2, real, c):
        c = c.to(self.device)
        if mode == "val":
        ...
        with torch.no_grad():
            ...
            for y_i in tqdm(y_range):
                ...
                for x_i in x_range:
                    ...
                    # モデルで予測を実行 (GPU上で計算)
                    predicted_patch = self.diffusion.sample(self.model, labels=patch, c=c,
                                                 num_inference_steps=self.num_inference_steps,
                                                 noise_add=self.noise_add, cfg_scale=self.cfg_scale)
                    predicted_patch_ema = self.diffusion.sample(self.ema_model, labels=patch, c=c,
                                                     num_inference_steps=self.num_inference_steps,
                                                     noise_add=self.noise_add, cfg_scale=self.cfg_scale)
        ...
        return np.clip(fake, -1, 1), np.clip(fake_ema, -1, 1), np.clip(real, -1, 1)

    def show_images(self, real, output_pred, output_pred_ema, c, mode, condition_n_x):
        ...
        plt.show()
        if mode == "val":
            wandb.log({f"{mode}_image/{self.target[c]}_normal": wandb.Image((output_pred + 1) * 0.5 * 255.0),
                       f"{mode}_image/{self.target[c]}_ema": wandb.Image((output_pred_ema + 1) * 0.5 * 255.0),
                       "epoch": self.epoch})
            if self.epoch == 0:
                # real = real.cpu().detach().numpy()
                wandb.log({f"{mode}_image/{self.target[c]}_target": wandb.Image((real + 1) * 0.5 * 255.0), })
        if mode == "test":
            wandb.log({f"{mode}_image/{self.target[c]}_{self.test_id}": wandb.Image((output_pred + 1) * 0.5 * 255.0),
                       f"{mode}_image/{self.target[c]}_ema_{self.test_id}": wandb.Image((output_pred_ema + 1) * 0.5 * 255.0),
                       "n": condition_n_x[c]})
            if self.test_id == "final":
                # real = real.cpu().detach().numpy()
                wandb.log({f"{mode}_image/{self.target[c]}_target": wandb.Image((real + 1) * 0.5 * 255.0),
