...
class Diffusion_ddim:
    def __init__(self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, device="cuda", noise_add=True, cfg_scale=3):
    ...
    def sample(self, model, labels, c, num_inference_steps=50, noise_add=None, cfg_scale=None):
        ...
        # model.eval()
        with torch.no_grad():
            B, C_cond, H, W = labels.shape
            x = torch.randn((B, 1, H, W)).to(self.device)
            ...
            for i in range(num_inference_steps -1):
                ...
                t_tensor = (torch.ones(B) * t).long().to(self.device)

                # print(x.shape)
                prediction = model(x, t_tensor, labels, c)
                predicted_noise = prediction[:, 0:1, :, :]
                if self.cfg_scale > 0:
                    uncond_prediction = model(x, t_tensor.to(self.device), labels, None)
    ...
    # condition, image_size修正の必要あり
    def sample_target(self, x, model, n, labels, num_inference_steps=50, noise_add=None, cfg_scale=None, target_steps=0):
        if cfg_scale is not None:
            self.cfg_scale = cfg_scale
        if noise_add is not None:
            self.noise_add = noise_add
        ...
        with torch.no_grad():
            x = x.to(self.device)
            t = torch.full((n,), target_steps).to(self.device)
            sqrt_alpha_hat = torch.sqrt(self.alpha_hat[t])[:, None, None, None]
            sqrt_one_minus_alpha_hat = torch.sqrt(1 - self.alpha_hat[t])[:, None, None, None]
            Ɛ = torch.randn_like(x)
            x = (sqrt_alpha_hat * x + sqrt_one_minus_alpha_hat * Ɛ).to(self.device)
            labels = labels.to(self.device)

            prediction = model(x, t, labels)
            output = 1 / sqrt_alpha_hat * (x - torch.sqrt(1 - sqrt_alpha_hat ** 2) * prediction[:, 0:1, :, :])
            # print(prediction.min(), prediction.max())
            # print(sqrt_alpha_hat[0])
            return output


    def sample_stop(self, model, n, labels, num_inference_steps=50, noise_add=None, cfg_scale=None, stop_steps=0):
        if cfg_scale is not None:
            self.cfg_scale = cfg_scale
        if noise_add is not None:
            self.noise_add = noise_add
        ...
        with torch.no_grad():
            ...
            for i in range(num_inference_steps -1):
                ...
                # alpha = self.alpha[t][:, None, None, None]
                alpha_hat = self.alpha_hat[t]
                alpha_hat_prev = self.alpha_hat[prev_t] #if prev_t >= stop_steps else torch.tensor(1.0)  # Handle the last step
