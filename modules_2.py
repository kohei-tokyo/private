...
class UNet_gemini(nn.Module):
    def __init__(
        ...
        # --- ラベル埋め込み (Label Embedding) ---
        # num_classes + 1 とするのは、Classifier-Free Guidance用に
        # 「Null(条件なし)」ラベルを含めるためです。
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList([
                    ResidualBlock(dim_in, dim_out, time_emb_dim=sum_dim),
                    ResidualBlock(dim_out, dim_out, time_emb_dim=sum_dim),
        ...
        # --- ボトルネック (Bottleneck) ---
        mid_dim = dims[-1]
        self.mid_block1 = ResidualBlock(mid_dim, mid_dim, time_emb_dim=sum_dim)
        self.mid_attn = SelfAttention(mid_dim)
        self.mid_block2 = ResidualBlock(mid_dim, mid_dim, time_emb_dim=sum_dim)
        ...
        for i in reversed(range(num_resolutions)):
            ...
            self.ups.append(
                nn.ModuleList([
                    ResidualBlock(res_in_ch, res_out_ch, time_emb_dim=sum_dim),
                    ResidualBlock(res_out_ch, res_out_ch, time_emb_dim=sum_dim),
    ...
    def forward(self, x_noisy, time, cond_image, c=None):  # cにデフォルト値Noneを設定
        # 1. 入力と条件画像を結合
        ...
        # 2.1. cと合成
        if c is None:
            # ラベル指定がない場合は、全て「Nullラベル(ID=num_classes)」とする
            c = torch.full((x_noisy.shape[0],), self.num_classes, device=x_noisy.device, dtype=torch.long)
        c_emb = self.label_emb(c)
        if self.condition_concat:
            t = torch.cat([t, c_emb], dim=1)
        else:
            t = t + c_emb
